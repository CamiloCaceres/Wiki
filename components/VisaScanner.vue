<template>
  <div>
    <div class="flex flex-col gap-4">
      <!-- Upload Form Group -->
      <UFormGroup label="Upload Visa Document" />
      ...
      >
        ...
        <UButton v-else :loading="isLoading" color="blue" variant="link" @click="open">{{
          isOverDropZone ? "Drop it!" : "Select or drop your visa document"
        }}</UButton>
      </div>
...
const emit = defineEmits<{
  "scan-complete": [visaData: VisaInfo];
}>();

// Drop Zone Setup
const { isOverDropZone } = useDropZone(dropZoneRef, {
  onDrop,
  dataTypes: ["application/pdf", "image/*"], // Allow both PDF and images
  multiple: false,
...
async function uploadFile() {
  ...
  try {
    const data = await $fetch<VisaInfo>(`${fastApiUrl}/extract/visa`, {
      method: "POST",
      body: formData,
      query: { token },
