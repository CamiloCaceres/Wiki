<template>
  <div>
    ...
    <div class="flex items-center space-x-4">
      ...
      >
      <UButton color="primary" @click="copyBachNurse"
        >Bach. Nurse Conditions</UButton
      >
    </div>
    ...
    <div>
      ...
      >
        <UHeading tag="h3" size="md" weight="medium">{{
          condition.name
        }}</UHeading>
    ...
    <div class="mt-4">
      <UButton color="primary" @click="generateConditions"
        >Generate Conditions</UButton
      >
    </div>
    <UModal
      v-model="showingConditions"
          ...
          <UCardBody>
            <div v-if="selectedConditions.length > 0">
              ...
              <ul class="list-disc pl-6 mb-4">
                <li v-for="condition in selectedConditions" :key="condition.id">
                  {{ condition.name }}
                </li>
              </ul>
...
const generateConditions = () => {
  selectedConditions.value = conditions.filter(
...
const copyToClipboard = () => {
  const conditionsList = selectedConditions.value
